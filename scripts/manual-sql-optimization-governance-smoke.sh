...
assert_post_json() {
  ...
  status_code="$(curl -s -o /tmp/sqlforge-sql-optimization-smoke-response.out -w '%{http_code}' \
    -X POST "$@" --data "${body}" "${url}")"
  if [[ "${status_code}" != "200" ]]; then
    echo "HTTP POST failed for ${url}, status=${status_code}" >&2
    [[ -f /tmp/sqlforge-sql-optimization-smoke-response.out ]] && cat /tmp/sqlforge-sql-optimization-smoke-response.out >&2
    exit 1
  fi
  cat /tmp/sqlforge-sql-optimization-smoke-response.out
}
...
assert_get_json() {
  ...
  status_code="$(curl -s -o /tmp/sqlforge-sql-optimization-smoke-response.out -w '%{http_code}' "$@" "${url}")"
  if [[ "${status_code}" != "200" ]]; then
