...
REQUEST_USER_ID="${REQUEST_USER_ID:-optimization-smoke-user-001}"
...
main() {
  ...
  SUCCESS_SUBMIT_REQUEST_ID="sql-opt-submit-success-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  SUCCESS_STATUS_REQUEST_ID="sql-opt-status-success-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  FAILURE_SUBMIT_REQUEST_ID="sql-opt-submit-failure-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  FAILURE_STATUS_REQUEST_ID="sql-opt-status-failure-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  COMPENSATION_REQUEST_ID="sql-opt-status-comp-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  COMPENSATION_TRACE_ID="${COMPENSATION_TRACE_PREFIX}-sql-opt-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)"
  ...
  mapfile -t stats_headers < <(build_protected_headers "sql-opt-stats-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)" "sql-opt-stats-$(TZ=Asia/Shanghai date +%Y%m%d%H%M%S)" "${issued_at}" "${expires_at}")
