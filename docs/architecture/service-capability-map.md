...
- benchmark task 现可持久化并查询 `scaleTarget`，用于声明 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标等生产规模意图，同时以 `TARGET_DECLARED_UNVERIFIED` 明确真实证明仍需报告和长窗口 workload evidence；`scaleTarget.evidenceManifest` 可保存外部生产规模证据引用、核验状态、`verificationBundle` 结构化核验包、`evidenceFileDigests` 原始文件摘要，以及 `environmentId/environmentType/evidenceOwner/artifactArchiveRef/verifierOperator` 来源元数据，并被 `scaleReadiness` 纳入 satisfied/missing evidence。只有 `externalVerificationStatus=VERIFIED`、bundle 同时覆盖 10000 并发、千万级日查询、30PB 字节级数据布局、24 小时 replay、P95/P99、扫描字节、CPU、队列等待、成本账单和 verifierRef，必需证据文件摘要完整，且来源元数据完整时，引用才可用于生产规模覆盖；未核验引用或单独 VERIFIED 字符串不能替代真实达标证明
...
- 下一轮后端实现优先级应是：
  ...
  4. 在 `governance` 上继续补租户 / 数据源访问范围校验与执行留痕链路
