...
- `acceleration_recommendation`
  ...
  - JSON 字段：`rule_chain_json`,`unapplied_rules_json`,`preconditions_json`,`semantic_risks_json`,`acceleration_artifact_json`,`expected_benefit_json`,`estimated_cost_json`
  ...
  - 高级 MV 产物：`acceleration_artifact_json` 保存推荐创建时的 canonical `accelerationArtifact` 快照；推荐详情与 diff 优先读取该快照，只有历史空快照行允许按 legacy 规则读时重算；响应层必须过滤 `EXACT_QUERY_MV` 或未知 `mvType`
