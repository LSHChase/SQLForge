...
当前 `benchmark_task.scale_target_json` 持久化压测声明的生产规模目标与 `TARGET_DECLARED_UNVERIFIED` 证据边界，用于把 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标接入任务状态、审计与 execution summary；该字段不是生产实测结论。
