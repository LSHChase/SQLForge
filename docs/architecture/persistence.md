...
当前 `benchmark_task.scale_target_json` 持久化压测声明的生产规模目标与 `TARGET_DECLARED_UNVERIFIED` 证据边界，用于把 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标接入任务状态、审计与 execution summary；其中 `evidenceManifest.verificationBundle` 可保存结构化核验包，但该字段仍不是生产实测结论。只有 bundle 逐项满足 10000 并发、30PB 字节级数据布局、24 小时 replay、P95/P99、扫描字节、CPU、队列等待、成本账单和 verifierRef，且外部状态为 `VERIFIED` 时，`scaleReadiness` 才会把它作为已核验生产规模证据。
