...
- 当前 `scaleTarget` 用于声明 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标等生产规模意图，并固定携带 `TARGET_DECLARED_UNVERIFIED` 证据状态；真实证明仍必须来自完成的 benchmark/report、P95/P99、扫描量、CPU/内存、成本账单或资源单元和长窗口 workload evidence，不能仅因目标字段存在而判定达标。
