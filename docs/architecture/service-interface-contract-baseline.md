...
| `GET /api/benchmark-engine/tasks/{taskId}` | path: `taskId` | `BenchmarkTaskStatusResponse` with `taskId`,`taskType`,`status`,`currentPhase`,`priority`,`progressPercent`,`targetEngines`,`scaleTarget`（含可选 `evidenceManifest` 生产规模证据引用、来源元数据、`verificationBundle` 结构化核验包与 `evidenceFileDigests` 文件摘要）,`templateId`,`templateType`,`templateVersion`,`testSetId`,`testSetSource`,`testSetLabels[]`,`testSetSourceRefs[]`,`readonlyRequired`,`shadowEnvironmentMode`,`desensitizationRequirement`,`thresholdCount`,`reportId`,`error`,`submittedAt`,`startedAt`,`finishedAt`,`queueMode`,`queueEvidence`,`contractStage`,`implementationStage` | `EXTERNALIZED_ARTIFACT_GOVERNANCE_TRACE_BASELINE` |
| `POST /api/benchmark-engine/tasks/recommendations/{recommendationId}/comparison` | path: `recommendationId`; `BenchmarkRecommendationComparisonCreateRequest` with `tenantId`,`benchmarkSqlRole`,`targetEngines[]`, optional `testSetName`,`templateId`,`templateVersion`,`concurrency`,`durationSeconds`,`rampUpSeconds`,`datasetSizeLabel`,`scaleTarget`（可携带 `evidenceManifest`：`concurrencyProofRef`,`dailyQueryVolumeProofRef`,`dataLayoutProofRef`,`workloadReplayProofRef`,`workloadReplayWindow`,`p95P99MetricProofRef`,`scanCpuQueueMetricProofRef`,`costBillProofRef`,`externalVerificationStatus`,`environmentId`,`environmentType`,`evidenceOwner`,`artifactArchiveRef`,`verifierOperator`,`evidenceFileDigests{fileName:{sha256,sizeBytes}}`,`verificationBundle{observedConcurrency,observedDailyQueryVolume,observedDatasetSizeBytes,workloadReplayDurationHours,p95LatencyMs,p99LatencyMs,scannedBytes,cpuUsagePercent,queueWaitMs,costBillAmount,costBillCurrency,verifierRef}`）,`thresholds[]`,`testSetLabels[]` | `BenchmarkRecommendationComparisonResponse` with `recommendationId`,`benchmarkSqlRole`,`testSet`,`benchmarkTask`,`contractStage`,`implementationStage` | `RECOMMENDATION_COMPARISON_ORCHESTRATION_BASELINE` |
...
- 当前 `POST /api/benchmark-engine/tasks/recommendations/{recommendationId}/comparison` 会显式读取 sql-optimization recommendation，校验 source/recommended SQL 均仍处于 benchmark 只读边界后，先生成 `RECOMMENDATION_GENERATION` test set（保留 source/recommended 双 case、recommendation/report/history/parse/sqlFingerprint 来源引用、风险标签和 `scaleTarget` 目标 evidence），再提交 `COMPARISON` benchmark task；该路径不会自动触发 recommendation 执行，也不会放行越界 SQL 进入 benchmark task。
- 当前 `scaleTarget` 用于声明 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标等生产规模意图，并固定携带 `TARGET_DECLARED_UNVERIFIED` 证据状态；`evidenceManifest` 保存外部生产规模证据引用、`externalVerificationStatus`、来源元数据与 `evidenceFileDigests` 原始文件摘要，覆盖并发证明、千万级日查询、30PB 数据布局、长窗口回放、P95/P99、扫描字节/CPU/队列等待和成本账单。`verificationBundle` 是结构化核验包，至少要逐项满足 10000 并发、千万级日查询、30PB 字节级数据布局、24 小时长窗口回放、P95/P99、扫描字节、CPU、队列等待、成本账单和 verifierRef，并且必需证据文件摘要与 `environmentId/environmentType/evidenceOwner/artifactArchiveRef/verifierOperator` 完整，`scaleReadiness` 才会接受 `VERIFIED` 作为 `productionExternalVerification`；真实证明仍必须来自完成的 benchmark/report 或已核验外部证据，不能仅因目标字段、引用或单独的 `VERIFIED` 字符串存在而判定达标。
...
- 当前报告模型已覆盖引擎指标快照、阈值判定、回归守护摘要、趋势图表、建议输出、执行摘要以及导出产物元数据；成功路径会生成 `JSON/PDF/HTML` 与 raw-data artifact，记录 `artifactKey/artifactKind/storageType/storageUri/storageEvidence/exportId/retentionDays/retentionPolicySource/retentionDeleteAfter`，并把 workload/backfill/compensation 结构证据与 provider-specific / multi-provider object-storage contract 写回报告和最小历史 evidence。产品化 artifact cleanup / recovery 操作面已移出当前项目边界。
- 当前 `REGRESSION_GUARD` 会把 failed threshold count 汇总为 `regressionSummary`；warning-only 场景保留在报告里，不再扩大到治理告警产品面。
...
- 当前 repo-local artifact lifecycle 已固化为保留当前 report-set、重写时清理陈旧 sibling 文件，以及在 `PDF/HTML/raw-data` 文件缺失时从持久化报告快照恢复后再继续返回响应；该能力属于 benchmark-engine 内部 artifact 读取韧性，不再暴露为治理侧恢复演练产品功能。
...
- 当前治理历史公开 trace lookup / detail 和 governance-triggered artifact cleanup / recovery operation contract 已从产品边界移除；SQL 执行历史和真实改写历史保留各自详情 evidence，不再提供独立追踪查询中心或治理侧 artifact operation API。
- 当前环境级 object-storage provider evidence 只作为 benchmark artifact storage / report evidence 保存；provider cleanup/delete 不再由治理产品面触发。
