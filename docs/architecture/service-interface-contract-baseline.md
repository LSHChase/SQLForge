...
- 当前 `scaleTarget` 用于声明 10000 并发、30PB 数据集、千万日查询、高复杂度和成本目标等生产规模意图，并固定携带 `TARGET_DECLARED_UNVERIFIED` 证据状态；`evidenceManifest` 只保存外部生产规模证据引用和 `externalVerificationStatus`，覆盖并发证明、30PB 数据布局、长窗口回放、P95/P99、扫描字节/CPU/队列等待和成本账单。真实证明仍必须来自完成的 benchmark/report 或已核验外部证据，不能仅因目标字段或未核验引用存在而判定达标。
