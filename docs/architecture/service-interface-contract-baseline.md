...
- 当前新增 `POST /api/governance/history/artifact-operations` 公共受保护入口，与 `POST /api/benchmark-engine/internal/artifact-operations` 内部受保护入口组成 governance-triggered cleanup/recovery operation contract；当前只开放 tenant/platform admin 触发，默认 cleanup scope 为 `MIRROR_ONLY`，不把 provider-backed object storage 写成仓库默认主路径。
