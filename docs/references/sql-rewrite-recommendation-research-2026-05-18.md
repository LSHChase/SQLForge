...
## Implemented Mapping In This Task

`SqlOptimizationPipelineService` now emits at least 36 distinct SELECT recommendation scenarios across safe, structural, and physical categories:
...
Coverage evidence is held by `SqlOptimizationPipelineServiceTest.shouldExposeAtLeastThirtySixSelectRewriteRecommendationScenarios`.
