...
- `deployments/`
  ...
  - `benchmark-production-evidence-runbook.md`：真实生产规模压测证据目录格式、`verify-benchmark-production-evidence.py` 校验入口、`audit-rewrite-production-readiness.py` 完成度审计入口、`verificationBundle` 与 `evidenceFileDigests` 生成边界。
