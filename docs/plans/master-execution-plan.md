...
| `PRW-006` | 实现改写记录发布、暂停和撤销接口 | 在 sql-optimization 中新增 publish/pause/unpublish 接口，状态动作先走领域状态机和统一授权入口，再调用 query-execution runtime binding；只有 runtime 返回 ACTIVE/PAUSED/UNPUBLISHED 后才回写 publishStatus、绑定 ID、规则版本和操作 trace。 | `PRW-004`,`PRW-005` | mvn -pl sql-optimization,query-execution,sqlforge-shared -am test、python3 scripts/foreman.py validate PRW-006 |
