...
| `PRW-006` | 实现改写记录发布、暂停和撤销接口 | 在 sql-optimization 中新增 publish/pause/unpublish 接口，只改变改写记录 publishStatus 并写入操作 trace；发布资格和 runtime binding 只作为独立参考或兼容追踪，不由状态动作触发。 | `PRW-004`,`PRW-005` | mvn -pl sql-optimization,query-execution,sqlforge-shared -am test、python3 scripts/foreman.py validate PRW-006 |
...
| `PRW-009` | 比对差异触发自动暂停与告警闭环 | 扩展周期验证服务，在改写结果不等价或超过容忍阈值时自动把改写记录置为 `PAUSED`，并写入告警、trace 和审计事件；不调用 runtime binding 流程。 | `PRW-006`,`PRW-008`,`HARN-136` | mvn -pl sql-optimization,query-execution,governance,sqlforge-shared -am test、python3 scripts/foreman.py validate PRW-009 |
