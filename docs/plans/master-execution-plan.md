...
| `HC-004` | 原始资料目录规则冲突 | `R-101`/阶段0计划要求空目录，但仓库已归档阿里原始资料 | 影响规则一致性与 lint 逻辑 | 记录冲突后由人类确认规则口径 | Open |
...
| `HC-007` | 交付闭环不完整 | `init-completion` 中存在 pending/planned 状态 | 影响 tag 和完成记录可信度 | 在 Phase-B 对齐真实交付记录 | Open |
