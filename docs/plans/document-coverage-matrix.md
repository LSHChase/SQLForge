...
| `docs/architecture/sql-rewrite-core-ir-architecture.md` | Authority | SQL 改写核心 L1-L5 IR 架构骨架、QBDAG 查询块分解、结构哈希、L4 关系代数改写候选、语义等价验证报告、抽象代价模型、帕累托改写排序、规则 DSL、规则冲突消解、层级契约与命名冲突处理 | Consumed | 约束 `USER-CN-REWRITE-CORE-IR-SCAFFOLD-20260521`、`USER-CN-QBDAG-DECOMPOSITION-PHASE1-20260521`、`USER-CN-RA-REWRITE-PHASE2-20260521`、`USER-CN-SEMANTIC-EQUIVALENCE-PHASE3-20260521`、`USER-CN-COST-BASED-REWRITE-PHASE4-20260521` 与 `USER-CN-REWRITE-RULE-ENGINE-DSL-CONFLICT-20260521` 的改写核心 IR 层级、QBDAG 分解、关系代数候选生成、语义等价验证、代价模型排序、规则 DSL、冲突消解、后端边界和与既有推荐规则等级的兼容方案 |
...
| `docs/quality/developer-copy-language-script-legacy-baseline.json` | Indexed | 仓库自研脚本英文 help / error / print / echo 历史存量基线 | Referenced | 支撑 `R-187` 至 `R-190` 中文化门禁，确保脚本英文说明存量只减不增 |
...
| `docs/references/raw-requirements/USER-CN-RA-REWRITE-PHASE2-20260521.md` | Archive | USER-CN-RA-REWRITE-PHASE2-20260521 raw requirement snapshot | Archived | 保存关系代数等价变换、CSE、纵向折叠、横向展开消除需求原文，供第二阶段实现、验证和后续 2.3.2 语义等价验证追溯 |
| `docs/references/raw-requirements/USER-CN-SEMANTIC-EQUIVALENCE-PHASE3-20260521.md` | Archive | USER-CN-SEMANTIC-EQUIVALENCE-PHASE3-20260521 raw requirement snapshot | Archived | 保存语义等价验证、差查询空结果判定、NULL 语义、SMT Solver 边界和聚合统计等价需求原文，供第三阶段实现、验证和后续 solver/schema 约束扩展追溯 |
| `docs/references/raw-requirements/USER-CN-COST-BASED-REWRITE-PHASE4-20260521.md` | Archive | USER-CN-COST-BASED-REWRITE-PHASE4-20260521 raw requirement snapshot | Archived | 保存抽象代价模型、Hetu/Presto 调整、帕累托前沿和 SLA 策略排序需求原文，供第四阶段实现、验证和后续真实统计接入追溯 |
| `docs/references/raw-requirements/USER-CN-REWRITE-RULE-ENGINE-DSL-CONFLICT-20260521.md` | Archive | USER-CN-REWRITE-RULE-ENGINE-DSL-CONFLICT-20260521 raw requirement snapshot | Archived | 保存规则 DSL、CSE-DEDUP-001 模板、规则依赖图、冲突类型和 Beam Search 消解需求原文，供规则引擎实现、验证和后续动态规则治理追溯 |
...
| `docs/references/raw-requirements/USER-CN-SIMPLIFY-LIFECYCLE-ACTIVATE-PAUSE-20260520.md` | Archive | USER-CN-SIMPLIFY-LIFECYCLE-ACTIVATE-PAUSE-20260520 raw requirement snapshot | Archived | 保存激活/暂停折叠需求原文，供影响分析、`INBOX-006` 人工决策和后续实现追溯 |
