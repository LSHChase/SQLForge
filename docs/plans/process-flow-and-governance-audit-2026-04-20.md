...
- 根级前端工程已存在并可构建
- Maven 聚合工程当前只包含 `sqlforge-shared/` 与 `governance/`
- `governance` 当前仅代表公共管理服务的阶段性基线，不代表 4 微服务已全部落地
...
- 主要要求与约束：
  - 任何新增模块必须标明其最终服务域
  - `governance` 只能向公共管理服务边界收敛
  - `sqlforge-shared` 只能承载真正公共能力
  - 不允许把查询执行、异步优化、压测主流程长期堆进 `governance`
- 验证锚点：
  - `R-120`
  - `R-126`
...
- `sqlforge-shared` 共享层做实
- `governance` 向公共管理服务收敛
- 访问控制完整代码化
...
- 风险：
  - 实现者可能继续把优化主流程堆进查询执行服务或 `governance`
- 建议：
  - 用显式勘误修订 `init.md` 第 4 部分与第 5 部分正文
  - 保留历史来源说明，但在当前执行口径中明确：
    - 查询执行服务：轻量解析、轻量改写、执行控制、运行时应用已激活加速配置
