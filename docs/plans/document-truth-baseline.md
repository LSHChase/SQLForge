...
- `sql-optimization/` 已具备 SQL 优化提交/轮询与数据库持久化基线：
  ...
  - SQL 改写核心 IR 已覆盖 QBDAG 查询块分解、L4 关系代数改写候选、候选级语义等价验证报告，以及基于 Scan/Shuffle/Compute/Memory 四维抽象代价的帕累托改写排序；当前仍是静态估算，不执行真实 SQL、不读取生产数据、不自动创建 runtime rewrite binding
