...
- 当前 CI 事实已形成统一文档落点：
  ...
  - `docs/deployments/phase-gate-baseline.md` 已把 `task_audit`、`compile-governance --check`、`workflow_dispatch` phase gate、`checkpoint/*` / `release.published` 自动 release gate，以及最小 R-118 校验收口为正式文档
- 当前前端业务页已消费多服务治理能力：
