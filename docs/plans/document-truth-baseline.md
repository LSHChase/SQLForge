...
- `governance/` 已具备最小治理基线：
  ...
  - `POST /api/governance/internal/acceleration-plan/trace/write` 已接入真实 `config_snapshot/execution_result/query_history` 追溯写入，用于承载 acceleration plan lifecycle evidence
...
- `query-execution/` 已具备查询执行边界基线：
  ...
  - 查询执行边界定义，显式固化路由、执行控制、轻量解析、轻量改写和已激活加速配置应用
  - `JDBC` / `REST` / `CLIENT` 三种 Hetu 访问模式的承载边界，以及 feature-flagged 的真实模式路由与结果聚合基线
  - 与 `governance` 的租户范围校验、数据源访问检查和审计写入 HTTP 客户端基线
  - 受保护内部 `/api/query-execution/internal/acceleration-plans/activate|pause` 契约，以及按 activated binding 控制 `PREFER_ACCELERATED` 的 no-fail-open runtime gating
...
- `sql-optimization/` 已具备 SQL 优化提交/轮询与数据库持久化基线：
  ...
  - SQL 改写核心 IR 已覆盖 QBDAG 查询块分解、L4 关系代数改写候选、候选级语义等价验证报告、基于 Scan/Shuffle/Compute/Memory 四维抽象代价的帕累托改写排序、静态规则 DSL / 规则依赖图 / Beam Search 冲突消解报告、Calcite 单解析栈融合与 Hetu 静态计划适配报告、最终 RewriteRecommendation 推荐输出与金融级排序/门禁报告，以及面向 `docs/test01.sql` 的核心算法链路一致性报告；当前仍是静态估算与规划，不执行真实 SQL、不读取生产数据、不调用真实 planner、外部 SMT Solver 或 RelToSqlConverter、不自动创建 runtime rewrite binding
...
- `benchmark-engine/` 已具备压测任务、报告查询与数据库持久化基线：
  ...
  - `benchmark-engine` 已通过 `governance` 内部受保护入口写入 `config_snapshot/execution_result/query_history/export_record` 追溯链；当前除报告/下载产物 export orchestration 外，还会把 workloadDigest/workloadSource/backfillApplied/workloadEvidence 作为显式结构字段沉淀到长期治理载荷，并把 compensation-replay 语义经 workload evidence / execution summary 持续写回治理追溯链
...
- SQL 优化服务已建立独立模块、提交/轮询 API、MySQL `optimization_task` / `acceleration_plan` 载体、scheduled worker，以及真实 SQL parser / AST analysis / conservative rewrite / acceleration suggestion pipeline；acceleration plan submit/activate/pause 治理闭环现已落地，但外部队列调度、回调通知，以及更接近 engine-native / materialized-view 的物理加速编排仍待 `Phase-D` 后续任务补齐。
