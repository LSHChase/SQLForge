...
4. `R-110` 仅在华为云 / 私有云生产部署场景中作为部署 runbook 前置条件；本地开发、普通任务和 repo-closed runtime smoke 不因未执行华为云部署而阻断。
5. `R-139` 阶段切换必须通过显式标准动作、workflow 或 release metadata 触发，不得仅凭聊天文本片段自动切换阶段。
