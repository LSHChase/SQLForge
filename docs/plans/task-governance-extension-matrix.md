...
| `HARN-070` | Resolved by user confirmation on 2026-05-06: materialize HARN-070 as a standard single-agent Main Foreman task bound to E-STORY-007 / Phase-E. Repository inspection will determine the final SQL input/output inventory, existing formatter/highlighter reuse, and concrete documentation entry points. Implementation must keep formatting/display separate from execution, query results, persisted history, and backend datasource semantics. | Expected data impact is low: changes target UI display/edit interactions and documentation. Potential risk exists if formatting is applied to persisted SQL, submitted SQL, query history records, or execution payloads; formal implementation must keep formatting/display separate from business semantics unless explicitly confirmed. | Rollback by reverting the single HARN-070 task commit if implemented as a normal task. Recovery should restore previous SQL input/output UI behavior, remove added formatter/highlighter dependencies or component wiring, and preserve task ledger/audit records according to SQLForge closeout rules. If a dependency is added, rollback must also remove lockfile/package changes tied only to this task. |
| `HARN-106` | 若要把本任务从落账扩大为直接改页面、跳过 `HARN-107` 至 `HARN-116` 的单任务拆分、复用已完成 `HARN-097` 至 `HARN-105` ID，或改变 `HARN-096` 的活动实现边界，需人工确认。 | 文档、任务台账、主计划与任务矩阵；不修改运行时页面、后端 API、持久化数据、SQL payload 或审计语义。 | 通过追加修正或回退本任务 commit 恢复落账前计划状态；若 ID 映射错误，保留历史完成任务不变并新增治理修正任务，不覆写 `tasks-done.md` 已归档记录。 |
| `HARN-107` | 若导航壳层重构需要删除既有路由、改变 legacy redirect、让 `/delivery-progress` 默认进入生产入口、或改变菜单可达性，需人工确认。 | 前端路由、导航状态和 workspace header；无后端持久化影响。 | 回退壳层抽离 commit，恢复 `App.vue`、`router/index.js`、`routePaths.mjs` 的既有路由和菜单行为；保留 contract 检查防止 path 漂移。 |
| `HARN-108` | 若共享布局抽象会引入新 UI 框架、重写设计 token、造成页面主流程卡片套卡片，或要求一次性改完全部业务页，需人工确认。 | 前端共享组件、样式契约和部分页面 layout；无持久化数据影响。 | 回退共享布局组件与引用点，恢复原页面样式；保留已证明无风险的 token/文档更新为后续小任务重新落地。 |
| `HARN-109` | 若概览页重构会把样本、窗口、session 或 PULL_ONLY 证据伪装成全局事实，或让临时交付页替代正式 Dashboard，需人工确认。 | Dashboard、Delivery、Runtime、Recovery 概览展示与前端状态；不写后端数据。 | 回退概览页结构变更，恢复原基于仓库真值的展示边界，并保留 sample/window/session/PULL_ONLY 明示文案。 |
| `HARN-110` | 若 SQL 查询/历史重构会改变 SQL 执行 payload、历史查询参数、原始 SQL 展示、导出/审计关联或后端权威边界，需人工确认。 | 查询页、历史列表、详情抽屉和 SQL 展示状态；不得修改历史持久化语义。 | 回退页面结构和 composable 变更，恢复 `SqlEditorField` / `SqlCodeBlock` 契约、历史查询参数和 raw SQL 不自动格式化行为。 |
| `HARN-111` | 若解析工作台拆分会改变 parser 算法、API payload、access parse 独立失败语义、或把静态解析写成真实执行证据，需人工确认。 | 解析工作台与统计页面展示、前端状态和详情弹层；不修改 parser 或后端数据模型。 | 回退页面拆分与展示层改造，恢复结构解析/access parse 分开展示和原 API contract；保留 SQL UI 契约测试。 |
| `HARN-112` | 若批量解析页面拆分会放宽 summary-first、大批量有限预览、失败记录详情入口或报表导入边界，需人工确认。 | 批量解析中心 UI、导入/详情弹层与 SQL 展示状态；不得改写批量持久化数据或解析口径。 | 回退批量中心结构变更，恢复有限明细预览、失败详情入口和既有 batch/report import contract。 |
| `HARN-113` | 若解析历史重构会改变默认空筛选、raw SQL 不自动格式化、历史分页/详情语义、issue-scene detail 或报表统计证据边界，需人工确认。 | 解析历史页面、详情弹层、报表统计与 SQL 展示；不迁移或重写历史数据。 | 回退解析历史页面变更，恢复空筛选、分页、详情、raw SQL 展示和报表统计 contract；必要时补充兼容测试而不是改写历史事实。 |
| `HARN-114` | 若资产、路由、推荐、压测或接入页面重构会把缺失写 API 做成伪可写、暴露只读证据外的内部字段、或混淆 environment-backed 与 repo-closed 事实，需人工确认。 | 多个业务页的列表、详情、placeholder 和证据展示；不修改后端接口或外部系统状态。 | 回退对应页面结构变更，恢复只读证据边界、placeholder 语义和缺失写 API 的显式提示。 |
| `HARN-115` | 若告警、取证、修复、故障处置或系统管理页面重构会弱化权限、脱敏、后端权威判断、trace lookup 或审计证据完整性，需人工确认。 | 运维/取证/系统管理页面 UI、查询状态与详情模式；不改变权限模型、审计表或后端 retry 行为。 | 回退页面结构和状态抽取，恢复权限/脱敏/审计边界，并保留 trace lookup、timeline、queue、retry 证据可见性。 |
| `HARN-116` | 若治理脚本加固会把现有页面债务一次性升为全仓阻断、误报未触碰页面、或放宽 R-177 至 R-184 已有要求，需人工确认。 | 前端治理脚本、设计文档和 validation 行为；不修改业务运行时数据。 | 回退脚本新增规则或改为 diff-aware 兼容策略，恢复 `npm run test:frontend-page-governance` 可执行性，并通过知识 lint 与任务审计证明治理链未被削弱。 |
| `HARN-120` | 若截图自检机器门禁会要求提交二进制截图入仓、阻断非页面类任务、或把无法重建 before 截图的历史修复任务一概判失败，需人工确认。 | 任务审计脚本、前端治理脚本、规则与计划文档；不修改业务运行时数据。 | 回退 `task_audit.py` 的 R-186 closeout 检查或缩小触发标记，保留文档规则；恢复 `npm run test:frontend-page-governance` 与 task audit 可执行性后再重新设计门禁。 |
| `HARN-127` | 若要把本任务扩大为直接实现业务代码、把当前目标方案写成已实现事实、跳过后续 `HARN-128` 至 `HARN-142` 单任务拆分，或改变 SQLForge 不直接装数/不直接改生产存储边界，需人工确认。 | 文档、主计划、任务矩阵、任务台账与原始需求快照；不修改运行时数据、后端接口实现、前端页面代码或数据库 schema。 | 通过追加修正或回退本任务 commit 恢复落账前文档和任务状态；若任务 ID 或边界有误，保留历史记录并新增治理修正任务，不覆写已归档内容。 |
| `HARN-143` | 若复核要求改变 HARN-127 的业务边界、把 `APPLIED` 当成默认运行时生效、把静态解析伪装成真实收益证据、把 `rewrite_validation_run` 迁到第二套真值服务，或取消 HARN-128 至 HARN-142 的逐任务实施顺序，需人工确认。 | 文档、接口/数据模型基线、主计划、任务矩阵、覆盖矩阵、任务台账与原始需求快照；不修改业务代码、数据库 schema、运行时配置或外部环境状态。 | 通过追加修正任务或回退本任务 commit 恢复 HARN-127 后的文档与任务状态；后续实现任务仍以复核后的矩阵依赖为准。 |
| `HARN-144` | 若复核要求改变业务实现范围、跳过 HARN-128 至 HARN-142、把文档一致性修正扩大为代码实现、或改变 SQLForge 不直接生产装数/不直接改存储边界，需人工确认。 | 文档、产品方案、接口/数据模型基线、主计划、任务矩阵、覆盖矩阵、任务台账与原始需求快照；不修改业务代码、数据库 schema、运行时配置或外部环境状态。 | 通过追加修正任务或回退本任务 commit 恢复 HARN-143 后的文档与任务状态；后续实现任务仍以 HARN-144 后的矩阵依赖为准。 |
| `HARN-128` | 若后端契约设计需要改变既有 acceleration plan 生命周期、推荐状态语义、租户隔离或 SQL 历史查询边界，需人工确认。 | DTO/VO、接口契约、状态枚举与最小服务骨架；不应迁移历史数据。 | 保留旧接口兼容层，回退新增契约或将新字段降级为可选。 |
| `HARN-129` | 若新增表或字段会破坏现有追溯链、引入物理外键、明文敏感字段或不可回退 DDL，需人工确认。 | `acceleration_candidate`、`sql_rewrite_record`、`rewrite_validation_run` schema 与 mapper。 | 提供兼容 DDL 回退或停用新增查询路径，保持旧推荐/加速计划查询可用。 |
| `HARN-130` | 若推荐模型把静态启发式写成真实执行收益、或允许高风险规则自动应用，需人工确认。 | 推荐输出 payload、rule chain、风险和验证字段。 | 回退高风险字段或默认 `MANUAL_REVIEW_REQUIRED`，保留旧推荐输出兼容。 |
| `HARN-131` | 若改写规则缺少元数据、唯一键、NULL、时区或排序语义证明却要自动应用，需人工确认。 | 改写规则、推荐 SQL 与风险标识；不直接执行推荐 SQL。 | 禁用相关规则或降级为人工复核候选，恢复原 SQL 输出。 |
| `HARN-132` | 若 SQL diff 会改变 SQL 文本、把展示 diff 写回推荐结果，或在前端承载权威语义判断，需人工确认。 | diff API、AST 摘要与展示用 payload。 | 回退 diff 服务或仅返回文本 diff，保持推荐主体不变。 |
| `HARN-133` | 若候选入口会混淆解析证据和真实运行指标、放宽 `sourceType/sourceKind/sourceId/evidenceLevel` 校验或跨租户聚合，需人工确认。 | acceleration candidate、source evidence、runtime evidence 与追溯键。 | 停用候选生成入口，恢复原优化任务和加速计划独立流程。 |
| `HARN-134` | 若 SQL 历史聚合会暴露跨租户改写记录、未脱敏 SQL 或隐藏失败验证证据，需人工确认。 | 改写记录、SQL 历史详情聚合和审计引用。 | 关闭聚合接口或回退到推荐弱引用展示，保留后端记录。 |
| `HARN-135` | 若结果比对需要执行非只读 SQL、拉取全量大结果到前端或改变生产数据，需人工确认。 | validation run、只读执行摘要、result digest 和差异样本。 | 禁用比对任务，保留 validation policy 草稿和旧推荐状态。 |
| `HARN-136` | 若调度或告警会产生过度噪声、自动回滚生产配置、或在未验证差异时暂停所有推荐，需人工确认。 | validation scheduler、alert event、auto-apply pause evidence。 | 关闭 scheduler 或缩小告警规则，恢复手动验证路径。 |
| `HARN-137` | 若工作台壳层会重复已有解析/推荐/历史完整页面、引入卡片套卡片或把未实现接口显示为成功，需人工确认。 | 前端路由、页面壳层、流程图、跳转和证据抽屉。 | 回退工作台页面或隐藏未实现操作，保留已有页面入口。 |
| `HARN-138` | 若真实接口按钮会绕过审批、自动应用生产配置、或把 mock 响应伪装成真实接口成功，需人工确认。 | 工作台 tabs、接口调用、状态映射和响应证据。 | 停用高风险按钮，恢复只读证据展示和跳转。 |
...
| `PRW-001` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 仅影响文档、主计划、接口契约说明和任务治理记录；不修改运行时数据、数据库 schema 或接口实现。 | 回退本任务文档增量并保留后续任务不执行；若状态命名不合适，用追加文档修正替代覆盖历史。 |
| `PRW-002` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增兼容性数据库字段、映射和 DTO 字段；旧数据必须有保守默认状态，不迁移为已审批或已发布。 | 通过兼容 DDL 或停用新增读写路径回退；保留旧改写记录查询能力，不删除历史记录。 |
| `PRW-003` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 更新改写记录审批状态和审计字段；不直接创建运行时绑定，不执行推荐 SQL。 | 停用审批接口并回退状态机服务；保留新增字段的保守默认状态，避免历史记录丢失。 |
| `PRW-004` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 读取验证、告警和改写记录状态并产生资格判断；不修改运行时绑定。 | 回退策略服务和发布入口调用，恢复为不可发布或只读状态。 |
| `PRW-005` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增运行时改写绑定持久化或等价存储结构；不改变当前 SQL 执行结果。 | 停用绑定仓储和服务入口，保持原 query-execution 执行路径不读取改写绑定。 |
| `PRW-006` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 发布时创建或更新运行时绑定并回写改写记录发布字段；暂停/撤销会改变自动改写生效状态。 | 暂停或撤销已发布绑定；回退发布接口后保持改写记录为未发布或暂停状态。 |
| `PRW-007` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 会改变命中绑定后的实际执行 SQL；必须保留原始 SQL 与绑定追踪，不改变未命中路径。 | 关闭绑定读取或将绑定置为 paused/unpublished，恢复原 SQL 执行路径。 |
| `PRW-008` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增或扩展执行历史追溯字段；旧历史记录必须有兼容默认值。 | 保留旧历史展示路径，停止写入新增改写审计字段或将其置为默认未改写状态。 |
| `PRW-009` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 差异验证会暂停 active 绑定并改变发布状态；必须保留告警和验证运行证据。 | 关闭自动暂停调度或回退暂停调用，恢复手动暂停路径；保留历史告警记录。 |
| `PRW-010` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 仅改变前端交互和 API 调用，不直接修改持久化数据；审批/发布状态由后端接口写入。 | 回退页面动作区和 API client 变更，恢复推荐中心只读或既有工作流显示。 |
| `PRW-011` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 仅改变历史页面展示和 API 消费，不改写历史持久化数据。 | 回退历史页面新增列、详情 tab 和跳转入口，恢复既有 SQL 历史视图。 |
| `PRW-012` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增测试、smoke、runbook 或契约检查；不改变业务生产数据。 | 回退新增测试和 smoke 接线，不影响 PRW-001 至 PRW-011 的已实现业务能力。 |
| `PRW-013` | 若实现需要绕过人类审批、等价验证、租户隔离、审计留痕、发布资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 同步 Redis 改写规则 key；主闭环状态仍以 query-execution 绑定为准，同步失败必须告警而非篡改主状态。 | 停用 Redis 同步适配并删除或标记无效的 Redis key，保持 query-execution 主绑定不变。 |
| `USER-CN-RECOMMENDATION-DIFF-LAYOUT-20260517` | 若实现需要改变数据库 schema、推荐状态机、diff 权威语义、审批/发布/dispatch/自动应用行为，或让前端排序字段直接进入 SQL 拼接，必须暂停并人工确认。 | 只新增后端查询面和前端展示/契约/测试；不新增表、不迁移数据、不改变 SQL 执行或推荐生命周期数据语义。 | 回退本任务单 commit 可恢复 legacy 推荐页布局和新增分页接口接线；保留旧 recommendations 数组接口不变以降低回滚风险。 |
