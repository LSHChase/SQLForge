...
| `HARN-143` | 若复核要求改变 HARN-127 的业务边界、把非 `ACTIVE` 状态当成默认运行时生效、把静态解析伪装成真实收益证据、把 `rewrite_validation_run` 迁到第二套真值服务，或取消 HARN-128 至 HARN-142 的逐任务实施顺序，需人工确认。 | 文档、接口/数据模型基线、主计划、任务矩阵、覆盖矩阵、任务台账与原始需求快照；不修改业务代码、数据库 schema、运行时配置或外部环境状态。 | 通过追加修正任务或回退本任务 commit 恢复 HARN-127 后的文档与任务状态；后续实现任务仍以复核后的矩阵依赖为准。 |
...
| `PRW-003` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 更新改写记录复核状态和审计字段；不直接创建运行时绑定，不执行推荐 SQL。 | 停用复核接口并回退状态机服务；保留新增字段的保守默认状态，避免历史记录丢失。 |
| `PRW-004` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 读取验证、告警和改写记录状态并产生资格判断；不修改运行时绑定。 | 回退策略服务和激活入口调用，恢复为不可激活或只读状态。 |
| `PRW-005` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增运行时改写绑定持久化或等价存储结构；不改变当前 SQL 执行结果。 | 停用绑定仓储和服务入口，保持原 query-execution 执行路径不读取改写绑定。 |
| `PRW-006` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 激活时创建或更新运行时绑定并回写改写记录激活字段；暂停会改变自动改写生效状态。 | 暂停已激活绑定；回退激活接口后保持改写记录为未激活或暂停状态。 |
| `PRW-007` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 会改变命中绑定后的实际执行 SQL；必须保留原始 SQL 与绑定追踪，不改变未命中路径。 | 关闭绑定读取或将绑定置为 paused，恢复原 SQL 执行路径。 |
| `PRW-008` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增或扩展执行历史追溯字段；旧历史记录必须有兼容默认值。 | 保留旧历史展示路径，停止写入新增改写审计字段或将其置为默认未改写状态。 |
| `PRW-009` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 差异验证会暂停 active 绑定并改变激活状态；必须保留告警和验证运行证据。 | 关闭自动暂停调度或回退暂停调用，恢复手动暂停路径；保留历史告警记录。 |
| `PRW-010` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 仅改变前端交互和 API 调用，不直接修改持久化数据；复核/激活状态由后端接口写入。 | 回退页面动作区和 API client 变更，恢复推荐中心只读或既有工作流显示。 |
| `PRW-011` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 仅改变历史页面展示和 API 消费，不改写历史持久化数据。 | 回退历史页面新增列、详情 tab 和跳转入口，恢复既有 SQL 历史视图。 |
| `PRW-012` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 新增测试、smoke、runbook 或契约检查；不改变业务生产数据。 | 回退新增测试和 smoke 接线，不影响 PRW-001 至 PRW-011 的已实现业务能力。 |
| `PRW-013` | 若实现需要绕过人工确认、等价验证、租户隔离、审计留痕、激活资格策略，或把投产前核验闭环混入生产闭环验收，必须暂停并回到人工确认。 | 同步 Redis 改写规则 key；主闭环状态仍以 query-execution 绑定为准，同步失败必须告警而非篡改主状态。 | 停用 Redis 同步适配并删除或标记无效的 Redis key，保持 query-execution 主绑定不变。 |
| `USER-CN-RECOMMENDATION-DIFF-LAYOUT-20260517` | 若实现需要改变数据库 schema、推荐状态机、diff 权威语义、复核/激活/dispatch/自动应用行为，或让前端排序字段直接进入 SQL 拼接，必须暂停并人工确认。 | 只新增后端查询面和前端展示/契约/测试；不新增表、不迁移数据、不改变 SQL 执行或推荐生命周期数据语义。 | 回退本任务单 commit 可恢复 legacy 推荐页布局和新增分页接口接线；保留旧 recommendations 数组接口不变以降低回滚风险。 |
