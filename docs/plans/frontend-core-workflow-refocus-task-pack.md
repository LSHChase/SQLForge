...
| 一级菜单 | 二级菜单 | 作用 | 边界 |
|:---|:---|:---|:---|
| 首页总览 | - | 展示核心 SQL 链路待办、最近查询、最近解析、推荐结果、改写风险摘要。 | 只做摘要和跳转，不承载审计详情、审批流、运行门禁操作。 |
| SQL 查询分析 | - | 提供 SQL 输入、格式化、校验、执行、Explain、查询结果、解析摘要、推荐入口、历史关联。 | 不做批量解析管理，不做审批发布，不替代 SQL 历史查询。 |
| SQL 历史查询 | - | 查询执行历史、筛选历史 SQL、查看执行摘要、原 SQL / 模板 SQL / 绑定后 SQL、关联解析、推荐和改写记录。 | 审计、告警、trace 只作为详情证据折叠展示，不作为主视觉。 |
...
| 辅助治理 | 追踪查询 | 按 traceId、taskId、historyId、recommendationId 查询链路证据。 | 只用于定位和取证，不定义业务主流程。 |
| 辅助治理 | 告警中心 | 查看告警、ACK、通知模拟和风险事件。 | 告警可关联核心页面，但告警中心不是核心功能入口。 |
| 辅助治理 | 运行门禁 | 查看运行时门禁、发布前风险与 smoke 证据。 | 运维辅助页面，不影响核心菜单排序。 |
...
**技术约束**：不得把 sample、window、session 或 PULL_ONLY 证据写成全租户事实；不得让审计、告警、门禁、恢复演练抢占核心首屏。
...
**接口契约**：不改变后端 recommendation diff 语义；不改变推荐状态、审批、发布、自动应用行为。

**技术约束**：dispatch、trace、审批证据进入折叠区或证据抽屉；前端只展示后端权威结果，不自行判断语义等价。
...
**人工确认点**：若要把推荐结果页面扩展为审批中心、发布中心或自动应用入口，必须人工确认。
...
**人工确认点**：若需要新增独立后端改写历史接口、改变审批/发布状态机或运行时绑定语义，必须人工确认。
