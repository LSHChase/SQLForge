...
**接口契约**：不删除推荐中心、SQL 历史、生产改写闭环、后端加速计划或运行时绑定能力；告警中心产品入口已移除，差异暂停 evidence 保留在推荐 / 改写 / SQL 历史主链路；只移除旧参考页入口与专属验证。

**技术约束**：主导航和 Dashboard 不得恢复旧流程模拟参考页或告警中心入口；核心验证继续通过推荐、历史和生产改写闭环 smoke 覆盖。
