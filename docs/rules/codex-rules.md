...
- `R-044`：影响实际使用者的运行假设必须落到测试、日志、配置或文档。
...
- `R-111`：所有受保护操作必须经过后端身份鉴别；核心引擎文档不再定义产品角色分层。
- `R-112`：必须基于租户 ID 做访问控制与数据隔离，禁止越权查询。
- `R-113`：所有 SQL 操作、登录登出和访问范围变更必须进入审计日志，保留不少于 180 天且不可改删。
...
- CRUD、配置、历史、任务、审计、数据源、访问范围、报表接口等管理后台页面默认采用：
  - 顶部搜索 / 筛选区
  - 主体操作按钮 + 表格或列表
  - 底部分页、批量状态或结果状态栏
...
- 前端不得自造受控访问结论，不得把按钮展示、禁用、隐藏或本地路由判断当作最终后端事实。
- 按钮可见性、禁用态和提示只允许作为体验层控制；后端仍是身份鉴别、访问判断、业务规则、数据隔离和审计的权威边界。
- 页面需要展示访问相关状态时，必须消费后端返回的能力、策略或错误结果；不得基于本地枚举、URL、租户输入或缓存推导最终访问结论。
...
- 缓存、字典、候选值、访问能力和跨页面共享状态应由 Pinia、composable 或后端接口统一承载，不得散落在页面临时变量中形成隐性事实源。
...
- 触发表单治理、日期区间、候选值、访问可见性或提交格式变更时，必须执行 `npm run test:form-governance` 或对应静态契约检查。
