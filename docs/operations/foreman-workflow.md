...
7. 将当前完成任务从 `tasks.md` 移入 `tasks-done.md`，确保 `Context closeout` 已完整；pre-commit 审计时只允许这一个最新归档任务尚未出现在 Git history
8. 运行 `python3 scripts/task_audit.py --check`
9. 只 stage 当前任务相关文件并创建单任务 commit
10. commit 完成后立即重新运行必要审计/知识校验，确认当前任务的 `Commit subject` 已进入 Git history，再执行“上下文清理”后进入下一任务；若当前 Codex 运行环境支持 `/contract`、`/clear` 或等价命令，可以使用，但它们只是可选实现方式，不是唯一规范动作
