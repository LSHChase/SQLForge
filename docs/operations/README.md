...
4. [Codex MCP Playbook](./codex-mcp-playbook.md)
   SQLForge 中使用 Codex + MCP 的只读接入边界、本地凭据/配置要求、onboarding / `mcp_doctor.py --check`、manifest-level `mcp_profile` 约束、自动化入口与证据写回规则。
...
- MCP 治理编译产物：`.codex/policy/mcp-policy.json`
