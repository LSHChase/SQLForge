...
- 触发时机：任务修改 Java 构建、测试、运行、CI workflow、部署文档、本地开发入口，或执行需要声明 Java 环境合规性的验证时。
- 检查清单：
  1. Java 编译、测试、运行、CI、测试环境和交付环境的版本声明均明确为 JDK 8u112。
  2. 执行 Java 相关命令前，如当前任务要把结果写成合规证据，必须保留 `java -version` 或等价环境版本证据，且版本为 `1.8.0_112`。
  3. CI workflow 或脚本若声明提供合规 Java 运行时，必须安装或校验 JDK 8u112，不得只使用浮动 `8`。
  4. 当前环境不是 JDK 8u112 时，相关 Java 验证只能记录为 skipped、blocked 或 residual risk，不能写成 JDK 8u112 合规通过。
- 通过标准：适用检查项全部通过。
