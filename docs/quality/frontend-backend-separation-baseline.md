...
- 当前仍需持续关注的点：
  ...
  - 开发态为了保持联调效率，Vite dev proxy 会基于 `X-SQLForge-Dev-*` 调试提示头注入临时受保护请求头；该逻辑仅允许停留在开发代理层，生产态仍必须由网关/反向代理负责
...
- 开发代理：若本地联调需要临时身份上下文，只允许在 `vite.config.js` 这类开发代理层完成，前端业务源码不得直接生成受保护头。
