...
| 2 | `推荐结果 / 改写记录` | 哪些推荐或改写治理对象可复核、激活、暂停 | `sql-optimization` 的 recommendation、`sql_rewrite_record`、`rewrite_validation_run` | 否 |
| 3 | `真实 SQL 改写历史` | 哪次生产 SQL 执行真的发生了自动改写，执行了什么 SQL，来源是哪条记录 | `query-execution` 写入的执行历史，由 `governance` 查询聚合 | 是 |

命名建议：

- `SQL 改写验证` 保留“验证/试算”语义。
- `推荐结果` 表示候选建议，不表示已激活或已生产执行。
- `改写记录` 表示可进入 activate / pause / validation 生命周期的治理对象。
...
- 只有 `ACTIVE` runtime binding 能表示运行时已生效。
...
- 核心推荐核验区：展示最终推荐报告、算法链路状态、置信度、风险、扫描减少估计和候选 SQL 形态命中，不替代原始证据。
- SQL diff 区：文本 diff、规则级 diff、AST 摘要差异。
- 验证区：验证方法、结果摘要、digest、差异样本、验证 run。
- 动作区：生成推荐、创建改写记录、重新验证、跳转推荐详情。

单条验证不应直接提供“生产生效”按钮。运行时生效只能走 `改写记录` 的激活接口。
...
批量能力应只做治理对象生成和验证，不自动批量激活生产改写规则。
...
- SQL diff。
- activation 状态、状态 trace、历史 runtime binding / 规则版本追踪字段。
- validation run 列表。
- 周期比对状态。
- 暂停、告警和 trace 证据。
...
| 解析推荐经激活为 runtime binding | `改写记录` | 改写记录详情展示 activation / binding 状态 | 否 |
...
3. 试算结果写入 `推荐结果`。
4. 用户采纳推荐并创建 `改写记录`。
5. 用户在 `改写记录` 中执行激活，后端完成激活资格校验并调用 runtime binding。
6. 激活成功后，改写记录 `activationStatus` 为 `ACTIVE`，并保留 activation evidence。
7. 激活和暂停均只通过后端状态接口变更改写记录状态并写入 trace。
...
- 展示 activation、validation、binding、pause 状态。
- 展示最近真实命中的 SQL 历史。
...
| 已验证 | validation run 通过 | 证明某次验证通过，不代表已激活 |
| 待激活 | `activationStatus=INACTIVE` | 不会被生产运行时使用 |
| 已激活 | `activationStatus=ACTIVE` 且 runtime binding `ACTIVE` | 表示后续同租户同指纹执行可命中运行时改写 |
| 本次已自动改写 | 执行历史 `rewriteApplied=true` 且有 binding 证据 | 生产事实 |
| 已暂停 | `activationStatus=PAUSED` | 改写记录状态暂停 |
...
5. 任何改写记录激活或暂停动作都必须在 `改写记录` 生命周期内完成，并通过后端状态接口记录 trace。
...
- 激活成功、运行时 active 和本次执行已改写三个状态不会混淆。
