...
本文是 `HARN-142` 的 repo-closed smoke 与文档收口入口，并承载 `PRW-012` 生产改写闭环 browser smoke。它覆盖加速治理工作台、推荐中心 SQL diff / 改写审批发布、SQL 历史改写记录追溯和告警联动的仓库内可复跑验证。
...
浏览器 smoke 会临时启动 Vite dev server，并在 Playwright 中 mock `/api/*`。它会覆盖候选创建、推荐任务、SQL diff、改写记录、计划提交/审批/应用/验证/回滚、基线与加速查询、推荐中心 SQL 改写审批/发布/暂停、SQL 历史改写记录回跳、runtime binding 版本证据、validation run 和 `SQL_REWRITE_RESULT_DIVERGENCE` 自动暂停证据。
